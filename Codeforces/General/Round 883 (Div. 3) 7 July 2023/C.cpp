...
bool cheking(pair<int ,int> t, pair<int, int> p){
    if(t.first == p.first){
        return t.second < p.second;
    }else return t.first > p.first;
}
...
void solve(){
    ...
    rep{
        ...
        score.push_back({solve,penalty});
        v.clear();
    }

    // cout << rs << " " << rp << endl;
    sort(score.begin(),score.end(), cheking);
    int dd = 0;
    for(auto t: score){
        dd++;
        if(rs == t.first && t.second  == rp){
            cout << dd << endl;
            return;
        }
    }
