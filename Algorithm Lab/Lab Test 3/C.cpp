...
void solve(){
    ...
    // }cout << endl;
    vector<int>porerdif,agerdif;
    rep{
        agerdif.push_back(a[i].first - b[i].first);
        porerdif.push_back(a[i].second - b[i].second);
    }
    dif = 1e9;
    rep{
        // cout << agerdif[i] << " ";
        if(agerdif[i] < dif)dif = agerdif[i];
    }
    // cout << endl;
    dif = dif * (-1);
    rep{
