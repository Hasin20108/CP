...
bool isPrime(int number) {
    ...
    for (int i = 5; i * i <= number; i += 6) {
        if (number % i == 0 || number % (i + 2) == 0) {
            return false; // Divisible by numbers of the form 6k ± 1
        }
    }
    return true;
}
void solve(){
    int n;cin >> n;
    int a[200001],b[200001],x;
    bool ok = false;
    int samne[200001],pichone[200001];
    rep{
        cin >> a[i];
        if(i == 0){
            samne[i] = a[i];
    ...
    int ans;
    if(n>2){
        ans = gcd(gcd(samne[0],pichone[2]),b[1]);
        for(int i = 1; i < n-2; i++){
            ans = max(gcd(gcd(samne[i],pichone[i+2]),b[i+1]), ans);
        }
        ans = max(pichone[0],ans);
        ans = max(ans,gcd(pichone[1],b[0]));
        ans = max(gcd(samne[n-2],b[n-1]),ans);
    }else{
        if(n == 1){
            ans = max(a[0],b[0]);
        }
        else ans = max(max(gcd(a[0],a[1]),gcd(a[0],b[1])), gcd(b[0],a[1]));
    }
    cout << ans << endl;
