...
void solve(){
    ...
    rep{
        if(ar[i].first%k == 0)ar[i].first = k;
        else ar[i].first = ar[i].first%k;
    }
    ...
    // vector<int> ans;
    // for(int i = 0; i < n; i++){
    //     if(ar[i].first % k == 0)ans.push_back(ar[i].second);
    //     else v.push_back(ar[i]);
    // }
    // for(int j = k-1; j >= 1; j--){
    //     for(int i = 0; i < v.size(); i++){
    //         if(v[i].first %k == j)ans.push_back(v[i].second);
