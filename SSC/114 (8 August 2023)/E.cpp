...
bool kop(pair<int,int>a,pair<int,int>b){
    if(a.first > b.first){
        return true;
...
bool ok(int k){
    ...
    int time = 0;
    for(int i = 1; i <= n; i++){
        if(a[i].second.first >= k && time + a[i].first <= t){
            time += a[i].first;
            cnt++;
        }
    }
...
void solve()
{
    ...
    int time = 0;
    for(int i = 1; i <= n; i++){
        if(a[i].second.first >= ans && time + a[i].first <= t && v.size() < ans){
            time += a[i].first;
            v.push_back(a[i].second.second);
        }
    }
    cout << v.size() << endl;
    for(int x: v) cout << x << " ";
