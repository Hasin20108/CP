...
bool comp2(pair<int ,int> t, pair<int, int> p){return t.second< p.second ;}
...
void solve(){
    ...
    sort(pir.begin(),pir.end(),comp);
    // sort(a,a+n);
    sort(b,b+n);
    // rep{
    //     cout << b[i] << " " << pir[i].first << ":" << pir[i].second << endl;
    // }

    rep{
        pir[i].first = b[i];
    }
    sort(pir.begin(),pir.end(),comp2);
