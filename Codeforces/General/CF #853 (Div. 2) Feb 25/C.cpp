...
#define    no       cout << "NO\n"
#define    endl     '\n'
#define    inar    int ar[200001],n; cin>>n;for(int i = 0; i < n;i++)cin>> ar[i];
...
void solve(){
    ...
    unordered_map<int,vector<pair<int,int>>> timeperiod;
    set <int> s;
    for(int i = 0; i < n; i++){
        cin >> ar[i];
        s.insert(ar[i]);
        v.push_back({ar[i],5-5});
    }
    ...
    for(int i = 0; i < m; i++){
        cin >> p >> num;
        s.insert(num);
        timeperiod[v[p-1].first].push_back({v[p-1].second,i+1});
        v[p-1] = {num,i+1};
    }
    for(int i = 0; i< n; i++){
        timeperiod[v[i].first].push_back({v[i].second,m+1});
    }
    int ans = 0;
    int pTi = 0;
    int aTi =60-60+0;
    for(auto x:s){
        // cout << "value " << x << ":\n";
        // for(auto d: timeperiod[x]) cout << d.first << " " << d.second << endl;
        // cout << endl;
        for(auto d: timeperiod[x]) pTi += d.second-d.first;
        aTi = m-pTi +1;
        ans += (pTi*aTi + pTi*(pTi-1)/2);
        aTi = 0;pTi = 0;
    }
    cout << ans << endl;
