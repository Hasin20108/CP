...
void solve(){
    ...
    for(int i = 0; i < q; i++){
        ...
        ar[r] = c;
        priority[c] = i;
        if(l < minirange) minirange = l;
        if(r > maxrange) maxrange = r;
    }
    // cout << maxrange << minirange << endl; 
    int x = ar[minirange];
    int boro = priority[x];
    // cout << l << r <<  endl;
    for(int i = minirange+1; i <= r; i++){
        // cout << i << " ";
        if(priority[ar[i]] < boro){
            ar[i] = x;
        }else if(priority[ar[i]] == boro){
            boro = -1;
            x = 0;
        }else{
            boro = priority[ar[i]];
    ...
    // cout << endl;
    // for(int i = minirange; i <= maxrange; i++){
    //     cout << i << " ";
    // }
    // cout << endl;
    for(int i = minirange; i <= maxrange; i++){
