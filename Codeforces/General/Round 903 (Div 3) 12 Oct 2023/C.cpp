...
void solve(){
    ...
    int ans = 1e15, temp1 = 0,temp2 = 0;

    // for(int i = 0; i < n/2; i++){
    //    ans += abs(s[i][i] - s[n-1-i][i]);
    //     ans += abs(s[i][n-1-i] - s[n-1-i][n-1-i]);
    // }
    // for(int row = 0; row < (n/2)-1; row++){
    //     for(int i = row+1; i < n-row-1; i++){
    //         ans += abs(s[row][i] - s[i][n-row-1]);
    //     }
    //     for(int i = row+1; i < n-row-1; i++){
    //         ans += abs(s[i][row] - s[n-row-1][i]);
    //     }
    // }
    // for(int i = 0; i < n-1; i++){
    //     for(int j = 0, k = i+1; j < n-i-1; k++,j++){
    //         temp1 += abs(s[i][j] - s[k][n-1-i]);
    //         // cout << i << " " << j << "      =     " << k << "  " << n-1-i << endl;
    //     }
    // }
    // for(int i = 0; i < n/2; i++){
    //     temp1 += (s[i][n-1-i] - s[n-1-i][i]);
    // }

    // for(int i = 0; i < n-1; i++){
    //     for(int j = 0, k = i+1; j < n-i-1; k++,j++){
    //         temp2 += abs(s[i][k] - s[k][i]);
    //         // cout << i << " " << j << "      =     " << k << "  " << n-1-i << endl;
    //     }
    // }
    // for(int i = 0; i < n/2; i++){
    //     temp2 += (s[i][i] - s[n-1-i][n-1-i]);
    // }

    for(int i = 0; i < n/2; i++){
        for(int j = i; j < n-i; j++){
            // cout << i << "," << j << "  " << n-j-1 << " " << i << endl;
            temp1 += abs(s[i][j]-s[j][n-1-i]);
            temp1 += abs(s[i][j]-s[n-i-1][n-j-1]);
            temp1 += abs(s[i][j]-s[n-j-1][i]);
        }
    }
    ans = min(ans,temp1);
    temp1 = 0;
    for(int i = 0; i < n/2; i++){
        for(int j = i; j < n-i; j++){
            // cout << i << "," << j << "  " << n-j-1 << " " << i << endl;
            temp1 += abs(s[i][j]-s[j][n-1-i]);
            temp1 += abs(s[j][n-1-i]-s[n-i-1][n-j-1]);
            temp1 += abs(s[j][n-1-i]-s[n-j-1][i]);
        }
    }
    cout << temp1 << endl;
    ans = min(ans,temp1);
    ...
    for(int i = 0; i < n/2; i++){    
        for(int j = i; j < n-i; j++){
            // cout << i << "," << j << "  " << n-j-1 << " " << i << endl;
            temp1 += abs(s[n-i-1][n-j-1]-s[j][n-1-i]);
            temp1 += abs(s[i][j]-s[n-i-1][n-j-1]);
            temp1 += abs(s[n-i-1][n-j-1]-s[n-j-1][i]);
        }
    }
        cout << temp1 << endl;
        ans = min(ans,temp1);
     ...
     for(int i = 0; i < n/2; i++){   
        for(int j = i; j < n-i; j++){
            // cout << i << "," << j << "  " << n-j-1 << " " << i << endl;
            temp1 += abs(s[n-j-1][i]-s[j][n-1-i]);
            temp1 += abs(s[n-j-1][i]-s[n-i-1][n-j-1]);
            temp1 += abs(s[i][j]-s[n-j-1][i]);
        }
     }
        cout << temp1 << endl;
