...
void solve(){
    unsigned int ar[2024];
    unsigned int x = 1;
    for(unsigned int i = 1; i <= 2023; i++){
        ar[i] = x;
        x += i;
        // cout << ar[i] << " ";
        // if(i%15 == 0) cout << endl;
    ...
    unsigned int n = 9;
    unsigned int a = 1,b;
    unsigned int up ,dow= ar[1];
    for(unsigned int i = 1; i< 2024; i++){
        if(ar[i] > n) {
            b = i;
            up = ar[i];
            break;
    ...
    unsigned int difleft = n - dow ;
    unsigned int difright = up - n - 1;
    unsigned temp = ar[a-(max(difleft,difright)) + 1] - 1;
    unsigned int ans = temp * (temp + 1)*(2*temp + 1)/6;
    unsigned int stop = n - dow + 1;
    for(int i = a-(max(difleft,difright)) + 1; )
    cout << ans << endl;
