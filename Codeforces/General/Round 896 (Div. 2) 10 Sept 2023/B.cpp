...
void solve(){
    int n,k,a,b,x,y,flag,ans,sum = 0;
    ...
    if(k >= 2){
        int da = 1e18 + 9999,db = 1e18 + 9999;
        for(int i = 0; i < k; i++){
            int temp = abs(v[i].first - v[a-1].first) + abs(v[i].second - v[a-1].second);
            if(temp < da){
                // na = i;
                da = temp;
            }
        ...
        cout << min(da + db,abs(v[a-1].first - v[b-1].first) + abs(v[a-1].second - v[b-1].second)) << endl;
    }else{
