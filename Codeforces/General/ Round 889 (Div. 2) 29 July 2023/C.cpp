...
ll neg() {
	for (int i = 1; i <= n; ++i) if (a[i] < 0) return i;
	return -1;
}
 
void printkor(int i, int j) {
	cout << i << " " <<  j << '\n';
}
void solve(){
    cin >> n;
	...
	if (p == -1) {
		cout << n -1 << '\n';
		for (int i = 1; i < n; ++i) printkor(i + 1, i);
	} else {
		cout << 8+ n * 2  << '\n';
		for (int i = 1; i <= 10; ++i) printkor(p, p);
		for (int i = 1; i <= n; ++i) if (i != p) printkor(i, p);
		for (int i = n - 1; i >= 1; --i) printkor(i, i + 1);
	}   
}   
