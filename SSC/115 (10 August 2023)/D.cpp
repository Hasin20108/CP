...
bool compareCups(const pair<int, int>& cup1, const pair<int, int>& cup2) {
    return cup1.second > cup2.second;
}

vector<int> distributeTea(int n, int w, vector<int>& cups) {
    vector<pair<int, int>> cupsWithIndices;
    for (int i = 0; i < n; ++i) {
        cupsWithIndices.push_back({i + 1, cups[i]});
    }

    sort(cupsWithIndices.begin(), cupsWithIndices.end(), compareCups);

    vector<int> allocatedTea(n, 0);
    int totalAllocated = 0;

    for (const auto& cup : cupsWithIndices) {
        int idx = cup.first - 1;
        int minTea = ceil(cup.second / 2.0);
        int allocated = min(minTea, w - totalAllocated);
        allocatedTea[idx] = allocated;
    ...
    if (totalAllocated < w) {
        for (const auto& cup : cupsWithIndices) {
            int idx = cup.first - 1;
            int cupVolume = cup.second;
            if (allocatedTea[idx] < cupVolume && totalAllocated < w) {
                int remaining = w - totalAllocated;
                int toFill = min(cupVolume - allocatedTea[idx], remaining);
                allocatedTea[idx] += toFill;
                totalAllocated += toFill;
            }
        }
    }

    for (int i = 0; i < n; ++i) {
        if (allocatedTea[i] < ceil(cups[i] / 2.0)) {
            allocatedTea.clear();
            allocatedTea.push_back(-1);
            break;
        }
    }
...
void solve(){
    ...
    vector<int> result = distributeTea(n, w, cups);
