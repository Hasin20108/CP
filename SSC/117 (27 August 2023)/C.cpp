...
void solve(){
    ...
    ll ans = n / 2;
    auto lo = lower_bound(all(powers), n+1);
    // cout << (*lo) << endl;
    if(lo == powers.end()) cout << "-1" << endl;
    else if(*lo != n+1) cout << "-1" << endl;
    else {
        if((ans ^ (ans + 1)) == n) cout << ans << endl;
        else cout << -1 << endl;
    }
...
int32_t main(){
    ...
    // freopen("/media/hasin/New Volume/DESKTOP/desktop files/CP/input.txt","w",stdout);
    #endif
    powers.push_back(1);
    for(int i = 1; i <= 31; i++) powers.push_back(powers[i - 1] * 2);
    int t = 1;  
    cin >> t;
    int i = 1;
    while(t--){
